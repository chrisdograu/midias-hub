      ...
      {titles.length === 0 ? (
        ...
        <div className="space-y-2">
          <button
            ...
            className={`w-full text-left px-3 py-2 rounded-lg border text-sm transition-all ${activeId === null ? 'border-primary bg-primary/10 text-foreground' : 'border-border bg-secondary/30 text-muted-foreground hover:border-primary/40'}`}
          >
            Nenhum título
          </button>
          {titles.map(t => (
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-lg border text-sm transition-all flex items-center justify-between ${activeId === t.id ? 'border-primary bg-gradient-to-r from-primary/15 to-accent/15 text-foreground' : 'border-border bg-secondary/30 hover:border-primary/40'}`}
            >
              <span className="font-medium gradient-text">{t.name}</span>
