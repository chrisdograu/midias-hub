      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Sparkles className="h-5 w-5 text-primary" /> Escolha até 3 gêneros favoritos
          </DialogTitle>
          <DialogDescription>
        ...
        <p className="text-xs text-muted-foreground">{picked.length}/3 selecionados</p>

        <DialogFooter className="gap-2">
          <Button variant="ghost" onClick={() => finish(false)} disabled={saving}>Pular</Button>
          <Button onClick={() => finish(true)} disabled={saving || picked.length === 0}>
            Salvar preferências
          </Button>
        </DialogFooter>
      </DialogContent>
