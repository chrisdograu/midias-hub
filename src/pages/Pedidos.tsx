      ...
      {pedidos.length === 0 ? (
        ...
        <div className="space-y-4">
          {pedidos.map(pedido => {
            ...
            return (
              <div key={pedido.id} className="bg-card border border-border rounded-xl p-4 space-y-3">
                ...
                {pedido.itens_pedido?.map(item => (
                  <div key={item.id} className="flex items-center gap-3 text-sm">
                    <img src={(item.produtos as any)?.image_url || '/placeholder.svg'} alt="" className="w-10 h-10 rounded object-cover" />
                    <span className="flex-1 text-foreground truncate">{(item.produtos as any)?.title || 'Produto'}</span>
